%%% Sin^2 Input
function [s,malphaf,malphae,malphai,v,rBalE,rBalI,rWBalE,rWBalI,rSimE,rSimI,xPlot]=AnisoSin2(T,N)
    ...
    Fe=FbarE*(c*sin(pi*xE).^2+(1-c)*sin(pi*xE));
    Fi=FbarI*(c*sin(pi*xI).^2+(1-c)*sin(pi*xI));
    ...
    % No balanced rates, set to zero
    rBal=zeros(2,numel(nxPlot));
    ...
    for m=1:2:maxn
        ...
        Ftildem=(m==1)*(1-c)/sqrt(2)+4*c*sqrt(2)/((4*m-m^3)*pi);

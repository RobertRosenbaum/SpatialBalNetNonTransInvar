...
function [s,malphaf,malphae,malphai,v,rBalE,rBalI,rWBalE,rWBalI,rSimE,rSimI,xPlot]=AnisoSin4(T,N)
    ...
    Fe=FbarE*(c*sin(pi*xE).^4+(1-c)*sin(pi*xE));
    Fi=FbarI*(c*sin(pi*xI).^4+(1-c)*sin(pi*xI));
    ...
    % The non-spatial, E/I part of r
    rBar=-inv(Wbar)*FBar;

    % The spatial part with no E/I separation
    rBalx=(1-c)*pi^2*sin(pi*xPlot)+2*c*pi^2*(cos(4*pi*xPlot)-cos(2*pi*xPlot));

    % Balanced rates
    rBal=rBar*rBalx;

    % Now do the same for the 'weak balance' case, for which we need to sum
    % the series
    maxn=21;
    rWBal=zeros(2,numel(xPlot));
    temp=0;
    rWBalphi4=[];
    for m=1:2:maxn
        % Eigenvalue, eigenfunction, and external input coefficient
        mum=1/(m^2*pi^2);
        phim=sqrt(2)*sin(m*pi*xPlot);
        Ftildem=(m==1)*(1-c)/sqrt(2)+48*c*sqrt(2)/((64*m-20*m^3+m^5)*pi);

        rWBal=rWBal+inv(epsilon*D-mum*Wbar)*FBar*Ftildem*phim;    
